...
@pytest.mark.parametrize(
    ...
    [
        revdeq.Simple(),
        revdeq.Relaxed(beta=0.8),
    ],
)
def test_implicit_adjoint(solver):
    function = F()
    args = jnp.array([0.1])
    adjoint = revdeq.RecursiveCheckpointAdjoint()
    loss, grads_recursive = grad_loss((function, args), solver, adjoint)

    adjoint = revdeq.ImplicitAdjoint(b_tol=1e-8, b_max_steps=500)
...
def test_phantom_adjoint():
    function = F()
    args = jnp.array([0.1])
    solver = revdeq.Relaxed(beta=0.8)
    adjoint = revdeq.RecursiveCheckpointAdjoint()
    loss, grads_recursive = grad_loss((function, args), solver, adjoint)

    adjoint = revdeq.PhantomAdjoint(beta=0.5, unroll_steps=50)
